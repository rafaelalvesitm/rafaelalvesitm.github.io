...
Aqui você encontra exemplos de código Markdown utilizado no [[Obsidian]].
...
```md
Link para uma página: [[Link interno]].
```

Os links internos são utilizados para linkar as notas entre si. 

---

### Incorporações

Incorpore outro arquivo (leia mais sobre [[Incorporar arquivos]]). Aqui está uma seção incorporada:

```md
![[Nome_da_nota ou imagem]]
```

As incorporações colocam o conteúdo de uma nota ou imagem no texto da nota atual. Voce pode utilizar:
- `![[Nome_da_nota#Nome_do_cabeçalho]]`  para incluir o texto dentro de uma seção de determinada nota. 
- `![[Nome_da_nota^ID_do_paragrafo]]` para incluir o texto de determinado parágrafo. 
...
##### Este é o cabeçalho 5
###### Este é o cabeçalho 6
```

Os cabeçalhos são utilizados para determinar as seções da sua nota. Eles utilizam um hierarquia de 1 a 6 (se você precisar de mais que isso você provavelmente deve refatorar as suas anotações)
...
_Este também é um texto em itálico
**Este é um texto em negrito**
__Este também é um texto em negrito__
_Você também **pode** combinar os dois_
```
...
```md
![Nome_do_arquivo](URL_do_arquivo)
```
...
A imagem acima pode ser redimensionada para uma largura de 100 pixels utilizando o filtro `|Largura` como indicado abaixo. 
